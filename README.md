...
Эта задача решается таким методом: сначала создает массив строк с примерами, затем вызывает метод FilterShortStrings, который фильтрует строки и возвращает новый массив коротких строк. Обратите внимание, что метод FilterShortStrings использует рекурсию для копирования коротких строк в новый массив.
